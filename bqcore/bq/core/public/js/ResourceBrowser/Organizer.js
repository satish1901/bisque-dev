    ...
    initFilters : function(uri) {
        ...
        for (var i = 0; i < this.tag_order.length; i++) {
            ...
            var filterCt = new Bisque.ResourceBrowser.Organizer.TagFilterCt({
                parent : this,
                tag_order : pair,
                tag_query : this.tag_query[pair[0]] || ''
            });
            this.add(filterCt);
...
Ext.define('Bisque.ResourceBrowser.Organizer.TagFilterCt', {
    ...
    GetTagList : function(loaded, type, tagData) {
        if (!loaded) {
            ...
            var uri = Ext.String.urlAppend(this.parent.dataset, '{0}=1&wpublic=' + this.parent.browser.browserParams.wpublic + (query.length ? '&tag_query=' + query : ''));
        ...
        this.tagCombo = Ext.create('Ext.form.field.ComboBox', {
            ...
            listConfig : {
                getInnerTpl : function() {
                    return ['<tpl if="value==&quot;gobject&quot;">' + '<div>' + '<p class="alignLeft">{name}</p>' + '<p class="alignRightGobject">gobject</p>' + '<div style="clear: both;"></div>' + '</div>' + '<tpl else>' + '<div>' + '<p class="alignLeft">{name}</p>' + '<p class="alignRightTag">tag</p>' + '<div style="clear: both;"></div>' + '</div>' + '</tpl>'];
                }
            },
