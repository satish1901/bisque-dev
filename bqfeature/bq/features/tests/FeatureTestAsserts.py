...
def isEqualXMLElement(element_a,element_b):
    ...
    if element_a.tag =='value':
        ...
        np.testing.assert_array_almost_equal(element_a_array,element_b_array, 4)
    else:    
...
def isCSVEqual(doc_a,doc_b):
    ...
    assert len(parsed_csv_a)==len(parsed_csv_b), '%s!=%s'%(parsed_csv_a,parsed_csv_b)
    
    #check if titles are the same
    assert parsed_csv_a[0]==parsed_csv_b[0], '%s!=%s'%(parsed_csv_a[0],parsed_csv_b[0])
    ...
    for row_a,row_b in zip(sorted_csv_rows_a,sorted_csv_rows_b):
        ...
        for ncol in range(1,len(row_a)):
            if parsed_csv_a[0][ncol]=='descriptor':
                ...
                assert row_a[ncol]==row_b[ncol], '%s!=%s'%(row_a[ncol],row_b[ncol])
...
#    asd
    with tables.open_file(path_hdf_a,'r') as h5file_a:
        with tables.open_file(path_hdf_b,'r') as h5file_b:
           ...
           assert set(table_a.colnames)==set(table_b.colnames), '%s!=%s'%(table_a.col_names,table_b.col_names)
           ...
           for idx_a,idx_b in zip(zip(*sorted_resource_a)[0],zip(*sorted_resource_b)[0]):
               for colname in table_a.colnames:
                   if colname == 'feature':
                       ...
                       assert table_a[idx_a][colname] == table_b[idx_b][colname], '%s!=%s'%(table_a[idx_a][colname],table_b[idx_b][colname])
