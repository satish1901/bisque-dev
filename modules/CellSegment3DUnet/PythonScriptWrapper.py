...
class PythonScriptWrapper(object):
	...
	hist_match.main(training_data_dir,testing_data_dir,hist_data_dir)	
	...
	log.info('seed index: %d, threshold: %f' %(seeds_slice_id, black_threshold))
	output_files = postprocessing.main(prob_map_datadir, results_outdir, seeds_slice_id, black_threshold)
	...
	try:
	    ...
	    self.preprocess(bq)
            ...
            self.bq.fail_mex(msg = "Exception during pre-process: %s" % str(e))
 	    return
        try:
	    bq.update_mex('Infer the images')
            ...
            self.bq.fail_mex(msg = "Exception during inference: %s" % str(e))
            return
        try:
	    bq.update_mex('Post process the images')
            self.outfiles = self.postprocess(bq)
        except (Exception, ScriptError) as e:
            log.exception("Exception during post-process")
            self.bq.fail_mex(msg = "Exception during post-process: %s" % str(e))
            return
	try:
            ...
            self.bq.fail_mex(msg = "Exception during upload result: %s" % str(e))
            return
