...
//--------------------------------------------------------------------------------
extern "C" {
__declspec(dllexport) extern void extractEHD(int *im, int rows, int columns, double *descriptor) {
...
__declspec(dllexport) extern void extractHTD(int *im, int rows, int columns, double *descriptor) {
	Mat Image( Size(rows,columns) , CV_64F );
    int i, j;
    for (i=0; i<rows; i++)
        for (j=0; j<columns; j++)
