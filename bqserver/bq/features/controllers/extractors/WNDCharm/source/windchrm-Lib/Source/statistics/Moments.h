...
//to get isnan working on MSVC10
#ifndef isnan
#ifdef _MSC_VER
#include <float.h>
  ...
  public:
	...
	void reset() { _mean = M2 = M3 = M4 = 0.0; _min = MAX_VAL; _max = MIN_VAL; _n = 0;}
  	inline double add (const double x) {
		...
		if (isnan(x) || x > MAX_VAL || x < MIN_VAL) return (x); //std::isnan
  	...
  	double skewness () const { return (_n > 3 ? (sqrt ((double)_n) * M3) / pow (M2, 1.5) : 0.0); } //type casted
