...
// 	max_pix_val = pow(2, bits) - 1.0;
	max_pix_val = pow((double)2, minbits) - 1.0;
...
// 		RGBwht[0] = RGBwht[1] = RGBwht[2] = max;
    ...
    // scale and write the deconvolved images.
		for (i = 0; i < 3; i++) {
			...
			img_stain8  = (uint8_t *)img_stain;
			img_stain16 = (uint16_t *)img_stain;
			img_stain32 = (uint32_t *)img_stain;
...
//				else output = (*pixP++) * max_pix_val;
			...
			// Write out the stain
			if ( (ret = WriteTIFFData (outfile,(unsigned char *)img_stain,bits,height, width)) != 0) {
