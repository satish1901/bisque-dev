...
// load gray scale images with any type
template<class T>
int ImageMatrix::LoadTIFFgraytemplate(TIFF* tif) {

    unsigned char spp = 1;
    T *buf = (T *)_TIFFmalloc((tsize_t)sizeof(T)*TIFFScanlineSize(tif)*spp); //creating buffer scaled to the file type size
...
// load color images with any type
template<class T>
int ImageMatrix::LoadTIFFcolortemplate(TIFF* tif) {
    ...
    for (int y = 0; y < height; y++) {
	    ...
	    while (x<width) {
		    ...
		    for (int sample_index=0;sample_index<spp;sample_index++) {
			    ...
			    T byte_data = buf[col+sample_index];
			    T val = T(byte_data);

                //assumes bits are always over 8 and there are 3 channels
                ...
                if (sample_index==0) rgb.r = T (R_stats.add (val));
				if (sample_index==1) rgb.g = T (G_stats.add (val));
				if (sample_index==2) rgb.b = T (B_stats.add (val));
...
//added by chris to read numpy images
...
int ImageMatrix::LoadTIFF(char *filename) {
    ...
    if( (tif = TIFFOpen(filename, "r")) ) {
        ...
        //conditions to import color vs gray

		// allocate the data 
		allocate (width, height);

        //import based on channels and bits per pixel
        if (spp==1){
            if (bits<=8) LoadTIFFgraytemplate<unsigned char>(tif);
            else if (bits<=16) LoadTIFFgraytemplate<unsigned short>(tif);
            else if (bits<=32) LoadTIFFgraytemplate<float>(tif);
            else if (bits<=64) LoadTIFFgraytemplate<double>(tif);
            else return(0);
        } else if (spp == 3) {
            if (bits<=8) LoadTIFFcolortemplate<unsigned char>(tif);
            else if (bits<=16) LoadTIFFcolortemplate<unsigned short>(tif);
            else if (bits<=32) LoadTIFFcolortemplate<float>(tif);
            else if (bits<=64) LoadTIFFcolortemplate<double>(tif);
            else return(0);
        } else return(0);
