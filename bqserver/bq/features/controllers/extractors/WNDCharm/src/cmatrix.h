...
public:
    ...
    template<class T>
    int LoadTIFFgraytemplate(TIFF* tif);

    template<class T>
    int LoadTIFFcolortemplate(TIFF* tif);
	...
	int SaveTiff(char *filename) {                   // save a matrix in TIF format
	    ...
	    for (y = 0; y < height; y++)
		    for (x = 0; x < width ; x++) {
			    BufImage[x + (y * width)] = T ( pix_plane (y,x) );
		    }
	    ...
	    for (y = 0; y < height; y ++) {
		    TIFFWriteScanline(tif,&(BufImage[y*width]), y,0 );
	    }
    ...
    //chris added function to feature_extractor
    void LoadImageMat(int *im, unsigned int height, unsigned int width, bool color);
