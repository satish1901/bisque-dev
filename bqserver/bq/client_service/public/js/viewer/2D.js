  ...
  if(!(b.x<=this.x||this.x+a.x<=0)){
    ...
    if(t>0){
      if(this.x<0){
        ...
        s=this.x*b.y;d=s/b.x-this.y
      }else{
        d=-this.y;
      }
    }else{
      if(b.x<this.x+a.x){
        s=(b.x-this.x)*a.y;d=b.y-(this.y+s/a.x);
...
Handle.prototype.mousedown=function(e){
    if(!this.locked){
        if(e.shiftKey){
            ...
            if(!this.selected){
                var owner=this.owner;mouser.unregisterAll();
                mouser.register(this);
            }
            mouser.beginDrag(e);
        }
...
LeverHandle.prototype.realize=function(){if(this.svgNode==null){var handle=document.createElementNS(svgns,"circle");var parent;if(this.owner!=null&&this.owner.svgNode!=null){parent=this.owner.svgNode.parentNode;}else{parent=_svgElement;}handle.setAttributeNS(null,"cx",this.point.x);handle.setAttributeNS(null,"cy",this.point.y);handle.setAttributeNS(null,"r",2.5);handle.setAttributeNS(null,"fill","black");handle.addEventListener("mousedown",this,false);parent.appendChild(handle);this.svgNode=handle;this.show(this.visible);}};
  ...
  while(!token.typeis(Path.EOD)){
    ...
    if(mode=="BOD"){
      if(token.text=="M"||token.text=="m") {
        index++;param_length=Path.PARAMS[token.text].length;mode=token.text;
      } else {
...
AbsolutePathSegment.prototype.realize=function(){
  for(var i=0;i<this.handles.length;i++){
    var handle=this.handles[i];handle.realize();
    handle.show(false);
  }
};
...
Pnt.prototype.selectHandles=function(select){this.p.select(select);};
Pnt.prototype.showHandles=function(state){this.p.show(state);};
