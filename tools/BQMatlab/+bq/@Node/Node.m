...
classdef Node < matlab.mixin.Copyable
    ...
    methods
        ...
        %     ~: empty (none value) remove share for that user
            ...
            if length(user)>8 && ...
               (strcmpi(user(1:7), 'http://') || strcmpi(user(1:8), 'https://')),
                user_res = bq.Factory.fetch(user, [], self.user, self.password);
            else
                url = bq.Url(uri);
            ...
            user_uniq = user_res.getAttribute('resource_uniq');

            % delete share
            if ~exist('mode', 'var') || isempty(mode) || strcmpi(mode, 'delete') == 1,
                % delete share
                [output, info] = bq.delete([uri '/auth/' user_uniq], [], self.user, self.password);
            elseif strcmpi(mode, 'read') == 1 || strcmpi(mode, 'edit') == 1,
                if isempty(sh),
                    % append share
                    doc = ['<auth user="' user_uniq '" action="' mode '" '  ' />']
                    [output, info] = bq.post([uri '/auth'], doc, self.user, self.password);
                else
                    % modify share
                    doc = ['<auth user="' user_uniq '" action="' mode '" '  ' />']
                    [output, info] = bq.post([uri '/auth' user_uniq], doc, self.user, self.password);
                end
            end
            if info.status>=300,
        ...
        function shares = getShares(self)
        % returns a list of users and their shares
            uri = self.getAttribute('uri');
